...
window.onload = function () {
	...
	myform.onsubmit = proverka
	/*привязываем событие onsubmit по наступлению которого 
...
function isMore() {
	...
	if (!document.getElementById("metka")) {
		/*условие по которому функция будет 
																	  определять открыта ли форма или нет*/
		for (var i = 1; i <= 6; i++) {
			/*если форма закрыта, то циклом по таймауту
			...
			setTimeout(p, t)
			/*запуск функций через определенное
																		количество времени*/
		}
		var myIn = document.getElementById("myIn");
		/*находим кнопку «расширенная форма» 
																							 по id*/
		myIn.setAttribute("value", "  Обычная форма  ");
		/*меняем название 
																			 кнопки на «обычная форма»*/
		myIn.removeAttribute("ONCLICK");
		/*удаляем атрибут ONCLICK, для того 
														чтобы в процессе выполнения кода сам 
														  код нельзя было запустить еще раз*/
	} else
		isNormal();
	/*если форма уже открыта, то запускаем функцию по удалению 
...
function isNormal() {
	...
	for (var i = 9; i < del.length; i--) {
		/*цикл по запуску функций для 
																			удаления полей, по таймауту*/
		if (i == 3) {
			/*если цикл доходит до 3 поля то он прекращается, 
														 чтобы у нас обязательные поля не удалились*/
			break;
		} else
			var fun = "deleteTR" + "(" + i + ")";
		/*формирование названия функции по 
...
function proverka() {
	...
	if (proverkaName.value == "" || proverkaEmail.value == "" || proverkaPsw.value == "") {
		...
		return false;
		/*прекращаем выполнения события onsubmit, 
...
сервере, т.е. настроить серверные скрипты");
		return false;
		/*для применения на действующем сайте эту строку необходимо удалить!!!
